...
import { therapists, therapistAvailability, therapistDocuments, users, profiles, reviews } from '../database/schema';
...
export const createTherapist = async (data: CreateTherapistData) => {
    ...
    const existing = await db.select().from(therapists).where(eq(therapists.userId, data.userId)).limit(1);
...
export const getTherapistById = async (therapistId: string) => {
    ...
    const [profile] = await db.select().from(profiles).where(eq(profiles.userId, therapist.userId)).limit(1);
