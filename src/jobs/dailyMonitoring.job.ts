...
export function startDailyMonitoringJob() {
  ...
  cron.schedule('0 0 * * *', async () => {
    ...
    try {
      ...
      const monitoredPatients = await db.select().from(patientMonitoring);
      ...
      for (const patient of monitoredPatients) {
        ...
        const recentMoodLogs = await db
          .select()
          .from(moodLogs)
          .where(and(eq(moodLogs.userId, patient.patientId), gte(moodLogs.loggedAt, oneDayAgo)));

        // Check mood trends
        const weekMoodLogs = await db
          .select()
          .from(moodLogs)
          .where(and(eq(moodLogs.userId, patient.patientId), gte(moodLogs.loggedAt, sevenDaysAgo)))
        ...
        if (recentMoodLogs.length === 0) {
          const thirtyDayLogs = await db
            ...
            .from(moodLogs)
            .where(
              and(eq(moodLogs.userId, patient.patientId), gte(moodLogs.loggedAt, threeDaysAgo)),
            );
        ...
        if (newRiskLevel !== patient.riskLevel) {
          ...
          if (newRiskLevel >= 2 && patient.therapistId) {
            ...
            logger.info(
              `[Job] Risk level increased for patient. Therapist notification would be sent.`,
            );
          }
        }
      }
