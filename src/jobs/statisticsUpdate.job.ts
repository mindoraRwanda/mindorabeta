...
export function startStatisticsUpdateJob() {
  ...
  cron.schedule('0 * * * *', async () => {
    ...
    try {
      // Get all therapists
      const allTherapists = await db.select({ id: therapists.id }).from(therapists);
      ...
      for (const therapist of allTherapists) {
        ...
        const [appointmentStats] = await db
          ...
          .from(appointments)
          .where(
            and(eq(appointments.therapistId, therapist.id), eq(appointments.status, 'COMPLETED')),
          );

        // Update therapist record
        const avgRating = reviewStats?.avgRating
          ? Math.round(Number(reviewStats.avgRating) * 10)
          : 0;
