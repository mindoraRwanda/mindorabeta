...
export const getAnalytics = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const [completedStats] = await db
    ...
    .from(appointments)
    .where(and(eq(appointments.therapistId, therapist.id), eq(appointments.status, 'COMPLETED')));
  ...
  const [monthlyStats] = await db
    ...
    .from(appointments)
    .where(
      and(eq(appointments.therapistId, therapist.id), gte(appointments.startTime, startOfMonth)),
    );
